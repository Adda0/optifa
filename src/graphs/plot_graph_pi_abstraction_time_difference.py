...
def plot_graph_scatter_comparison(df: pd.DataFrame, fig_location: str = None, show_figure: bool = False,
    ...
    sbg = sns.relplot(x=f"{test_type.value}.L.smt_free.mean", y=f"{test_type.value}.P.no_lengths.mean", data=df, legend=False)
    ...
    max_val_axis = max(df[f"{test_type.value}.L.smt_free.mean"].max(), df[f"{test_type.value}.P.no_lengths.mean"].max()) * 1.3
    sbg.set(
...
if __name__ == "__main__":
    ...
    plot_graph_scatter_comparison(df, fig_location=f"graph_la_pi_et_scatter_mean.pdf", show_figure=False,
                                  test_type=TEST_TYPE.ET)
    plot_graph_scatter_comparison(df, fig_location=f"graph_la_pi_fp_scatter_mean.pdf", show_figure=False,
