...
def plot_graph_scatter_comparison(df: pd.DataFrame, fig_location: str = None, show_figure: bool = False,
    ...
    sbg = sns.relplot(x=f"{test_type.value}.B.states", y=f"{test_type.value}.C.states", data=df, legend=False)
    ...
    max_val_axis = max(df[f"{test_type.value}.B.states"].max(), df[f"{test_type.value}.C.states"].max()) * 1.3
    sbg.set(
...
if __name__ == "__main__":
    ...
    plot_graph_scatter_comparison(df, fig_location=f"graph_pi_pruning_et_scatter.pdf", show_figure=False,
                                  test_type=TEST_TYPE.ET)
    plot_graph_scatter_comparison(df, fig_location=f"graph_pi_pruning_fp_scatter.pdf", show_figure=False,
