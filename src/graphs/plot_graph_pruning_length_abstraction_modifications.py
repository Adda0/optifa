...
def plot_graph_scatter_comparison(df: pd.DataFrame, fig_location: str = None, show_figure: bool = False,
    ...
    sbg = sns.relplot(x=f"Product Basic", y=f"Product Optimized", data=df, legend=False)
    ...
    max_val_axis = max(df[f"Product Basic"].max(), df[f"Product Optimized"].max()) * 1.3
    sbg.set(
...
if __name__ == "__main__":
    ...
    plot_graph_scatter_comparison(df_et, fig_location=f"graph_la_pruning_et_scatter_modifications.pdf", show_figure=False,
                                  test_type=TEST_TYPE.ET)
    plot_graph_scatter_comparison(df_fp, fig_location=f"graph_la_pruning_fp_scatter_modifications.pdf", show_figure=False,
