...
def plot_graph_scatter_comparison(df: pd.DataFrame, fig_location: str = None, show_figure: bool = False):
    ...
    df_et = df.filter(["ET.L.smt_free.states", "ET.C.states"], axis=1)
    df_et.rename(columns={'ET.L.smt_free.states': 'FP.L.smt_free.states', 'ET.C.states': 'FP.C.states'}, inplace=True)
    df_fp = df.filter(["FP.L.smt_free.states", "FP.C.states"], axis=1)
    ...
    sbg = sns.relplot(x=f"FP.L.smt_free.states", y=f"FP.C.states", data=df_fp, legend=False)
...
if __name__ == "__main__":
    ...
    plot_graph_scatter_comparison(df, fig_location=f"graph_pi_la_comparison.pdf", show_figure=False)
