...
def plot_graph_scatter_comparison(df: pd.DataFrame, fig_location: str = None, show_figure: bool = False):
    ...
    df_et = df.filter(["ET.L.smt_free.mean", "ET.B.mean"], axis=1)
    df_et.rename(columns={'ET.L.smt_free.mean': 'FP.L.smt_free.mean', 'ET.B.mean': 'FP.B.mean'}, inplace=True)
    df_fp = df.filter(["FP.L.smt_free.mean", "FP.B.mean"], axis=1)
    ...
    sbg = sns.relplot(x=f"FP.B.mean", y=f"FP.L.smt_free.mean", data=df_fp, legend=False)
    ...
    max_val_axis = df[f"FP.L.smt_free.mean"].max() * 1.3
    sbg.set(
...
if __name__ == "__main__":
    ...
    plot_graph_scatter_comparison(df, fig_location=f"graph_basic_la_time_difference.pdf", show_figure=False)
